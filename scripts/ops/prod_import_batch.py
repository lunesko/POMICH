#!/usr/bin/env python3
"""Import Ukraine directory providers on prod city-by-city (avoids OOM)."""
...
def main() -> int:
    ...
    for city in CITIES:
        ...
        _, rc = run(client, cmd, timeout=300)
        if rc != 0:
            failed.append(city)
            print(f"[WARN] import failed for {city}, restarting app...")
            ensure_app(client)
        time.sleep(1)
    ...
    summary = {"before": before, "after": after, "failedCities": failed, "health": health_out.strip()}
